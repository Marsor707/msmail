...
export async function getValidAccessToken(email: string, forceRefresh = false) {
  ...
  if (tokenResponse.scope && !hasMailReadScope(tokenResponse.scope)) {
    throw appError(
      502,
      'TOKEN_SCOPE_INVALID',
      '当前 refresh_token 换取到的不是 Graph Mail.Read 令牌，请确认原始授权包含 Mail.Read',
      {
  ...
  const nextExpireAt = new Date(now + tokenResponse.expires_in * 1000)
